...
class Bridge(Infra):
    ...
    def __init__(self, unique_id, model, length=0,
        ...
        self.breakdown_prob = self.model.scenario[self.condition]
        self.delay_time = 0

    def bridge_break_down(self):
        """
        Returns whether the bridge breaks down at the current tick
        __________
        Parameters:
            self
        __________
        Return: boolean - True if the bridge breaks down at the current tick. False otherwise
        """
        breakdown_probability = self.model.random.random()
        if breakdown_probability < self.breakdown_prob:
            return True
        else:
            return False

    def generate_delay_time(self, bridge_breakdown):
        """
        Generates the self.delay_time based on length of bridge
        __________
        Parameters:
            self
            bridge_breakdown: boolean - True if the bridge breaks down. False otherwise
        __________
        Return: None
        """
        if bridge_breakdown:
            if self.length < 10:
                self.delay_time = int(self.model.random.uniform(10, 20))
            elif self.length < 50:
                self.delay_time = int(self.model.random.uniform(15, 60))
            elif self.length < 200:
                self.delay_time = int(self.model.random.uniform(45, 90))
            elif self.length >= 200:
                self.delay_time = int(self.model.random.triangular(60, 120, 240))
        else:
            self.delay_time = 0

    def update_bridge_status(self):
        """
        Updates the bridge breakdown status and delay time

        """
        bridge_breakdown = self.bridge_break_down()
        self.generate_delay_time(bridge_breakdown)
        if bridge_breakdown:
            self.model.record_bridge_breakdown(self)

    def get_delay_time(self):
        """
        Return self.delay_time in minutes
        __________
        Parameters: self
        __________
...
class Sink(Infra):
    ...
    def remove(self, vehicle):
        self.model.removed_vehicles.append(vehicle)
...
class Vehicle(Agent):
    ...
    def __init__(self, unique_id, model, generated_by,
        ...
        self.travel_distance = 0 # keep track of travel distance
        self.removed_by = None # destination
        # self.travel_time = None  # travel time is created during vehicle removal
    ...
    def drive(self):
        ...
        if distance_rest > 0:
            ...
            self.travel_distance += self.location.length - self.location_offset # keep track of travel distance
            self.drive_to_next(distance_rest)
        else:
            # remain on the same object
            self.location_offset += distance
            self.travel_distance += distance # keep track of travel distance
    ...
    def drive_to_next(self, distance):
        ...
        if (isinstance(next_infra, Sink)) and (next_infra.unique_id == self.path_ids.iloc[-1]):
            # if vehicle arrives at sink location at sink location is the destination sink of its selected route then remove vehicle.
            self.arrive_at_next(next_infra, 0)
            self.removed_at_step = self.model.schedule.steps
            self.removed_by = next_infra
            # self.travel_time = self.removed_at_step - self.generated_at_step  # calculate travel time
        ...
        if next_infra.length > distance:
            ...
            self.travel_distance += next_infra.length  # keep track of travel distance
    ...
    def arrive_at_next(self, next_infra, location_offset):
        ...
        self.location_offset = location_offset
        self.location.vehicle_count += 1
        self.travel_distance += location_offset  # keep track of travel distance

    def calculate_travel_time(self):
        """
        Calculate the total travel time of a vehicle when it arrives at a sink
        ____________
        Parameters: Self
        Return: int - total travel time of a vehicle.
        """
        if self.removed_at_step is not None:
            return self.removed_at_step - self.generated_at_step
        else:
            return self.model.schedule.steps - self.generated_at_step

    def get_destination_road_name(self):
        """
        Returns the road name of the destination
        ____________
        Parameters: Self
        Return: str - destination road name. Returns 'en_route' if the vehicle hasn't arrived at a sink
        """
        if self.removed_by is not None:
            return self.removed_by.road_name
        else:
            return "en_route"
    ...
    def get_travel_distance(self):
        ...
        ____________
        Parameters: Self
        Return: int - total travel in meters.
        """
